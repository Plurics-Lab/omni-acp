 ...
 *
 * Two configs, one runner. `agents.ci.yaml` is hermetic — the SDK example agent and the ten
 * testkit fixtures — and runs everywhere. `agents.local.yaml` names REAL agents and runs only
 * under `OMNI_COMPAT_REAL=1`, because a machine without `claude-acp` logged in must report a
 * printed SKIP rather than a red build.
 *
