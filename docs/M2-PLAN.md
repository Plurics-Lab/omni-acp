...
### 5.2 What M2-WP-J actually did, where it differs from this plan, and why

The join found things the packages could not, because it is the first place they run together and the
first place they meet a real agent. Each row is a decision with a reason, recorded here so a reader of the
plan and a reader of the tree see the same milestone.

| # | deviation | why |
| - | --------- | --- |
| 1 | `RunUtility`'s options gained ONE optional field, `env` (`protocol/contracts.ts`, `core/process/spawn.ts`) | D8's technique is `GIT_INDEX_FILE=$tmp git add -A`, and `GIT_INDEX_FILE` has **no command-line spelling**: the temp index that keeps us off the operator's own index is reachable only through the environment. Every other flag the provider needs is argv (`-C`, `--no-ext-diff`, `--no-textconv`), so no `cwd` seam was taken. Additive and optional, so `platform-windows.ts` and every fake still satisfy the type |
| 2 | `DiffProvider.begin` returns a SENTINEL handle for "not a repo" and "git missing" | `worker.ts` is frozen and calls `end` only for a non-null handle (hunk 8), so a `null` from `begin` drops the `omni/patch` key and takes the WARNING with it — and §25.3's "`null` is an answer, and it is always explained" would be false for exactly the two cases it names first. The only `null` from `begin` is `diff.mode:"off"` |
| 3 | `diff.mode`'s `on_write` and `always` agree, and the gate is the two TREES rather than the turn's tool calls | `worker.ts` hands the provider no view of the turn, and F38 records codex reporting a two-file read as one `kind:"read"` call — a tool-call gate would under-report exactly the turns `always` exists for. Equal trees mean nothing was written, answered on disk truth, and `end` then skips `diff-tree`. **AMENDED in review round 2 (finding V12).** The two modes agreed on the answer and were byte-identical in COST, so `on_write` — the documented default — saved nothing and §11.9's stated mitigation was fiction: `cfg.mode` was read in exactly one place, to answer `"off"`. `worker.ts` now DOES hand the provider a view of the turn, and the deviation's own reasoning is what shapes it: the observation is folded off the NORMALIZED envelopes (any `edit`/`delete`/`move` tool call, or any `diff` content block), it is deliberately GENEROUS for F38's reason, and an absent report is read as "it wrote". The tree comparison stays exactly where it was — it is what makes the ANSWER right; `wroteFiles` is what makes the second `git add -A` + `write-tree` pair skippable |
| 4 | `registry.ts` DECORATES the handle it returns, and `WorkerSnapshot`'s M2 rows are stamped there | They are facts about the REQUEST (`onUnresolved`, `policy`, `mcp`, `envKeys`, `patchMode`, the park fields) and `worker.ts` is frozen with nowhere to hold them. `http/routes/workers.ts` serializes `handle.snapshot()` directly, so the `201` body would otherwise disagree with `GET /v1/workers/{wid}` |
| 5 | `RehydrateDeps` gained the same six optional seams, and the registry rebuilds them from the ROW | §5.8.8 persists `onUnresolved` / `env` / `mcpNames` / `policyRef` "BECAUSE OF THE WAKE PATH". A `park` worker that woke without its strategy would stop declaring `clientCapabilities.elicitation` — F28's silent degradation to prose, arriving through the one door nobody watches. **AMENDED in review round 2 (findings V2/V8 and V3).** It rebuilt the STRATEGY and not the ENVIRONMENT: no `decide`, no `mcpServers`, and a containment gate bound to the raw `config.tokens[].cwdRoots` instead of to the resolved `AuthContext` list. And the row itself was never written — `worker-store.ts` had no column for any M2 field — so "from the ROW" was reading defaults. `SCHEMA_VERSION` 3 adds `workers.m2_json`, `WorkerRow` gains the `PolicySelection`, and the wake re-resolves the engine and the presets from the CURRENT config or refuses with `acl_revoked` |
| 6 | the policy engine runs for EVERY create, so an auto-denied permission now reports `rule:"deny-all#default"` where M0 said `m0:auto-deny` | `PolicyCeiling.park:false` is enforced in the same call, and a worker admitted without it would be a ceiling nobody checked. The decision and the `optionId` are unchanged; two M1 integration assertions carry the new string with the reason inline. §4 step 7's "`two-workers-do-not-interfere.itest.ts` untouched" is therefore *almost* true: the file is untouched except for that one string, and no assertion was removed or relaxed |
| 7 | `CompatContext.serverUrl` became a getter; both watchdog cases create a DEDICATED worker; `permission-allow`, `patch-*` and `watchdog-cancel` create theirs under an explicit policy | Found by running them. `withDaemonConfig` restarts the daemon: the port changes (so a captured url is closed) and the shared worker is CLOSED (so its budgets are a previous configuration's). And the daemon's default preset is `deny-all`, so a case that needs the agent to write or to run a shell was asserting the DENY path under an allow's name |
| 8 | the elicitation cases carry their OWN prompt, transcribed from claude `12` | `prompts.remember` asks a question the agent can simply answer, so no real agent elicits for it. The fixtures elicit unconditionally, which is exactly why only a real run could find this |
| 9 | `agents.ci.yaml` gains `fixture-stall-in-tool` and NOT the four `elicit-*` | `watchdog-cancel` needs an agent that leaves a tool call open, and the stall fixture is it. The elicit fixtures cannot satisfy `elicitation-gated`: its control asserts that an agent told nothing about elicitation does not send one (F28, a REAL agent's gate) and a fixture sends one unconditionally. Listing one bought two `config` skips and a five-minute timeout |
| 10 | a new fixture, `patch-writer.mjs`, and `initGitRepoAt` on the testkit | No agent available to a hermetic test writes a file: the SDK example agent asks permission and then SIMULATES the edit. A patch is a fact about the filesystem, so D8 was untestable below the real-agent tier without one. The same fixture creates `.git/` mid-turn, which is the only deterministic reproduction of F39 |
| 11 | `known.ts` ships a SECOND builtin, `codex-acp` | Acceptance 8 asks for codex's `cmd`-matching gap in the descriptor, and a runtime with no descriptor has nowhere to put one. Every row cites its transcript; the load-bearing ones are `permission` and `elicitation` heading its `unverified` list, which is what makes every interaction case a printed `capability` skip for that agent rather than a silent pass |
| 12 | four hand-built `as AuthContext` doubles in `daemon/test` were completed with the M2 members | The creation path calls all three on every create, so a double that omitted them would only prove the cast still compiles. Each answers what the real one answers for a request that asked for nothing, and throws otherwise |

### 5.3 Review round 2 — the twelve post-integration findings

`docs/review/2026-09-09-m2-integration-review.md` is the input; its `处理记录` section is the per-finding
record. What changed structurally, so a reader of this plan does not have to open the review:

| # | change | files |
| - | ------ | ----- |
| V1 | `POST …/{deliveryId}/redeliver` goes through `Daemon.dispatcher`, the only object that re-runs `assertWebhookUrl` before a replay; `WebhookDispatcher.redeliver(id, tokenId?)` carries the D13 scope | `protocol/contracts.ts`, `core/webhook/dispatcher.ts`, `daemon/create-daemon.ts`, `daemon/http/routes/webhooks.ts` |
| V2 / V8 | the wake path rebuilds the policy engine and the MCP presets from the persisted `PolicySelection` / `mcpNames` against the CURRENT config, or closes the worker `acl_revoked`; `WorkerRow`'s M2 half is finally PERSISTED (`SCHEMA_VERSION` 3, `workers.m2_json`) | `daemon/registry.ts`, `core/persist/{schema,worker-store}.ts`, `protocol/contracts.ts` |
| V3 | one spelling of `cwdRoots`: the wake path reads the token store's resolved `AuthContext`, and the structural audit now checks EVERY `validateContent` site | `daemon/registry.ts`, `core/test/worker/prompt-content.test.ts` |
| V4 | §19.7 rule 1 / rule 5 are enforced at the point that EMITS the answer, for the injected strategy as well as the baseline | `core/worker/worker.ts` |
| V5 / V7 | the IPv4-mapped fold in `toBytes` is NUMERIC, and `0.0.0.0/8` joins the default `denyCidrs` | `core/webhook/guard.ts`, `protocol/config.ts` |
| V6 | `runs` / `deliveries` / `dispatcher` refuse with a typed `OmniError` after `stop()` instead of throwing raw sqlite errors | `daemon/create-daemon.ts` |
| V9 | `policy_clamped`, `policy_blinded`, `interaction_declined`, `interaction_expired` and `unpoliced_tool_call` are produced; the park path carries its clamp; `WorkerSnapshot.policyBlinded` is set | `protocol/turn.ts`, `core/worker/worker.ts`, `core/worker/interaction/{strategy,registry}.ts`, `core/policy/{engine,ceiling,alert}.ts` |
| V10 | §19.6's check order is the order the code runs: state → existence → lease → shape → semantics | `core/worker/worker.ts`, `daemon/registry.ts`, `daemon/http/routes/interactions.ts` |
| V11 | `daemon.stop()` settles interactions FIRST and drains a second time after the workers close | `daemon/create-daemon.ts`, `daemon/registry.ts`, `core/worker/worker.ts` |
| V12 | `diff.mode:"on_write"` skips the second `git` pair on a turn that wrote nothing | `core/worker/worker.ts`, `core/diff/git-provider.ts` |

Two contracts moved DOWN a layer, and for one reason: `reduceTurn` is the only caller that can put a warning
on a turn without breaking D7, and `@omni-acp/protocol` may not import `@omni-acp/core`. `policyClampWarning`
and `unpolicedToolCalls` are now declared in `protocol/turn.ts` and re-exported unchanged from
`core/policy/{ceiling,alert}.ts`, so nothing that imported them has to move.
