...
| 3 | `diff.mode`'s `on_write` and `always` agree, and the gate is the two TREES rather than the turn's tool calls | `worker.ts` hands the provider no view of the turn, and F38 records codex reporting a two-file read as one `kind:"read"` call — a tool-call gate would under-report exactly the turns `always` exists for. Equal trees mean nothing was written, answered on disk truth, and `end` then skips `diff-tree` |
