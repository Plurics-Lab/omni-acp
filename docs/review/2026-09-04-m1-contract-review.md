...
## 第二轮评审（2026-09-04，8 条）与处理记录（commit `chore: M1 contract review follow-ups (round 1)`）

第一轮修法之后的复审，针对**已落地的代码与文档**。八条全部处置：**applied**（照建议改）/
**amended**（改了，形状与建议不同，写明理由）。全仓 `pnpm -r build && pnpm test` 绿：
**980 passed / 2 skipped / 75 todo**（多出的一个 todo 是新增的 WP‑B 验收 7），`pnpm lint` 与
`prettier --check` 干净。**依然没有实现任何 WP 行为**：所有 `unimplemented: M1-WP-x` 桩仍然抛错，
本轮落地的只有三个缝各自缺掉的那一半，加上一个 §5.7 声明过、Land 步骤漏掉的桩。

### 逐条

| # | 结论 | 处置 | 落在哪里 |
| - | ---- | ---- | -------- |
| 1 | blocker | **applied** | 缝 2 的 rehydration 半边。`worker.ts`：`class Worker` **导出**（不上 `@omni-acp/core` 的 barrel——`exports-are-stable` 的集合不变），构造函数增加可选第二参 `restore?: { row: WorkerRow }`，按行 seed `#state` / `#emittedState` / `#sessionId` / `#capabilities` / `#currentTurnId` / `#closeReason` / `#generation` / `#crashed` / `#hibernatedAt` / `#wakeCount` / `#wakeFailures` / `#resume` / `#orphan` 与 row 的时间戳；`closed` 行**预先 resolve `#closePromise`**（`row.closeResult`），于是 `close()` 逐字节重放而不是重算 `treeGone`（§15.6 第 3 级）。`closeResult` 缺失（某次启动死在 close 中途）走**悲观**兜底 `{leaderExited:false, treeGone:false, sessionClosed:false, reason: closeReason ?? "orphaned"}`——`treeGone: true` 是 §6.6 唯一禁止的乐观。无 `restore` 参数时构造函数与 M0 逐行相同，**没有一个 M0 测试挪动**。文档：M1-PLAN §1.2 记为「open + reopen + **restore**」并写明 `worker.ts` 又吃了一次 Land 编辑，§1.1 的注释改成「Land-edited（seams 1 + 2），然后冻结」，§1.4 第 12 条；CONTRACTS §5.7 的 `worker/` 块加上这个类与构造签名；`worker/rehydrated.ts` 的注释指向 `new Worker(deps, { row })` 并写明 `RehydrateDeps`（WP‑C 自己的类型）可以自由生长。 |
| 2 | blocker | **amended** | 缝 1 的输入侧。建议的三件事全做了，形状上多了两个必需的配套。(1) 新增 `#runCloseOut(budgetMs)`：喂 `close_requested`，等 reducer 报 `settled`，由 `#afterStep` 在 settle 的那一刻唤醒；`#doClose`（在 `session/close` **之后**、kill **之前**，且 `force` 时**跳过**——`force` 的含义就是「每个协作梯级都已经给过了」，§6.5）、`#doHibernate`（回收进程前）、`cancel()` 的升级定时器（原先直接跳到第 5 级）三处调用。(2) `#watchProcess` 的 `stdoutEnded` 分支喂 `drained`。(3) `#openProcess` 订阅 `proc.stderr.onLine` 喂 `stderr_line`，unsubscribe 存在 `#stderrUnsub`，`#reclaimProcess` / `#doClose` 里调用。**与建议不同的两处，都是「不这样就跑不起来」**：① `#rescheduleTick` 的 tick 投递原本被 `#closing` 完全压住，而梯子**靠 tick 前进**——`#closeOutActive` 为真时不再压制，否则每一次梯子都会挂到兜底超时；② 建议里的 “bounded by `hardMs`” 在 Worker 里不可见（`hardMs` 是 `createNormalizer` 的选项），所以新增可选 `limits.closeOutMs`（缺省 10 s 常量），并在注释与 CONTRACTS §5.7 里写明它是**兜底**而不是梯级期限，必须**大于** reducer 自己的预算，WP‑E 传 `hardMs + drainGraceMs + cancelGraceMs`。三个输入统一走新的 `#feedLadder`：M0 的 reducer 对它们抛 `unknown turn input`，`#feedLadder` 返回 `null`、**debug** 级记一次（不是 `#feed` 的 `error`），调用方原样落回 M0 路径——这就是这次改动在 M0 切片下行为中性的原因。`stderr_line` 第一次被拒就退订，避免一个狂写 stderr 的 agent 刷屏。文档：M1-PLAN §1.2 缝 1 新增「INPUT side」段，§1.4 第 12 条；CONTRACTS §5.7 在 `CreateWorkerDeps` 后增加一段点名三个生产者；新增 **WP‑B 验收 7**「梯子在**真实 Worker** 上端到端跑通」（原 7–10 顺延为 8–11），`core/test/normalizer/m1-acceptance.test.ts` 同步加一条 `it.todo`。 |
| 3 | blocker | **applied（并且一次做完）** | 缝 3 的 epoch。`contracts.ts`：`ClientRef.epoch?: number` 与 `AuthContext.leaseEpoch: number | null`。**没有停在类型上**：`daemon/src/auth.ts` 的 `verify()` 现在解析 `HEADER.leaseEpoch`（`^\d+$` + `Number.isSafeInteger`，非法值 ⇒ `bad_request`，因为「被悄悄忽略的 fence 比没有 fence 更糟」），`asClientRef()` 把它 spread 进去（没有时**省略**而不是写 null，`epoch?: number` 的含义就是「没送」）；`contextFor()`（进程内、没有 header）恒为 null；testkit `stubAuthContext` 补 `leaseEpoch: null`。**所有冻结签名一字未动**：`prompt(content, who)` / `cancel(who)` / `wake(who, opts)` / `assertHolder(who, opts?)` 原样，只是**值**多了一个位置可以搭车。WP‑D 的 `createLease` 在 `assertHolder` 里读 `who.epoch ?? opts?.epoch`；`alwaysGrantedLease` 忽略它，所以默认仍是 M0。文档：M1-PLAN §1.2 缝 3 新增「And the epoch」段；CONTRACTS §5.4 的 `AuthContext` 补字段。 |
| 4 | minor | **applied** | `registry.delete()` 改为 `if (auth.role !== "admin") entry.handle.lease.assertHolder(auth.asClientRef());`，注释写明这是 §16.1 L3 的另一半、以及为什么它必须在 registry 而不在 lease 里（lease 不知道是谁在问、带什么权限，这也正是 `lease()` façade 把 `admin` 传给 `steal` 的原因）。M1-PLAN 的 **WP‑D 验收 2** 补上「一个不持有 lease 的 admin 在 `DELETE` 上**仍然成功**」；§1.2 seam 3 那一行也点了这个 guard。 |
| 5 | minor | **applied** | 三处 `supervisor.live.size` 断言改成 compat 套件真的观测得到的形状：M1-PLAN §4 Step 3（先从快照里读 `process.pid`，再断言 `process === null` + `await waitGone(pid)`）、§4 Step 6（每个 worker 的终态快照 `process === null`，脚本记下的每个 pid 都 `waitGone`）、CONTRACTS §18.4 的 `hibernate-wake` 行。三处都写明了原因（`createSupervisor` 在 `@omni-acp/core`，`tests/compat` 不依赖它；`createDaemon` 从不暴露自己的 supervisor）以及「死掉的 pid 比缩小的 map 是更强的证据」。**没有**给 `tests/compat` 加 `@omni-acp/core` 依赖——那会把一个只为断言存在的 test-only seam 变成常设依赖。WP‑C 验收 4 的 `supervisor.live.size === 0` **保留**：那是 core 的单测，`fakeSupervisor()` 就在手边。 |
| 6 | major | **applied（推荐方案：划归 WP‑E）** | `normalizer/vendor/registry.ts` 归 **WP‑E**：M1-PLAN §3 的 WP‑B 行改成 `packages/core/src/normalizer/**`（**minus `vendor/registry.ts`**），WP‑E 行与 WP‑E 的 “Owns exclusively” 块都列出这个文件并写明**为什么不搬走**——它是按 descriptor 键控、按进程学习 `-32601` 的（§17.3），留在 `normalizer/**` 下面才继续被 `descriptor-is-the-only-branch` 守卫扫到。WP‑B 的描述从「the vendor registry's dialects」改为明确的「两个 vendor **dialects**（`vendor/dialects.ts`）」。落地的桩标签 `M1-WP-E` 无需改动。 |
| 7 | minor | **applied** | `runEventLogPersistenceConformance` 按 §5.7 的签名补进 `packages/testkit/src/event-log-conformance.ts`（`make: typeof tmpPersistence`，体是 `unimplemented: M1-WP-A`），并加进 `tests/integration/src/exports-are-stable.itest.ts` 的 `@omni-acp/testkit` 冻结名单——该文件用 `toEqual` 做集合相等，所以这一条是被机器检查的。桩的注释写明它为什么是**独立函数**而不是 `runEventLogConformance` 的一个 flag（M0 那套必须逐字不改地跑在两个 driver 上，那才是「环还在」的证明）。M1-PLAN §1.4 第 12 条记下这次遗漏。 |
| 8 | minor | **applied** | `tests/compat/vitest.config.ts` 的头注释与 `tests/compat/src/compat.ctest.ts` 的第一条 `it.todo` 都改成「八个能跑完一个回合的 fixture（`crash` 与 `orphan` 被排除）」，并写明理由（套件的第一条断言就是「一个回合结束了」）。顺带修掉同一处漂移的第三个副本：CONTRACTS §5 目录树里 `agents.ci.yaml` 那一行。 |

**没有削弱任何测试**：本轮没有改动任何断言，新增的只有一条 `it.todo`（WP‑B 验收 7）和一个导出名。
